...
def create_svm_model(kernel, balancing, C, num_roll):
    train = construct_df(range(2009, 2019), n=num_roll, main_path=main_path)
    test = construct_df([2019, 2020], n=num_roll, main_path=main_path)
    ...
    if kernel == "rbf":
        clf = SVC(random_state=18, kernel=kernel, class_weight=balance_param, C=C)
    else:
...
for i, row in opt_models.iterrows():
    ...
    print(create_svm_model(row["param_kernel"], row["balancing"], row["param_C"], row["roll_num"]))