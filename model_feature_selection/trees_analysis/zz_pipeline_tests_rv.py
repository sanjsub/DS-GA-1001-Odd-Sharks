...
def rf_model_bakeoff():
    ...
    for ind, datagroup in enumerate(holder):
        ...
        param_dict_rf['datagroup'] = 3 - ind
        best_model_to_csv(param_dict_rf, 'zzzrfll_rv')
        res = pd.DataFrame(rf.cv_results_)
        df = pd.concat([df, res])
        df.to_csv("zzzrfll_cv_rv.csv")
        joblib.dump(rf, 'grid_rf_rv{}.pkl'.format(3-ind))
...
def gbt_model_bakeoff():
    ...
    for ind, datagroup in enumerate(holder):
        ...
        param_dict_gbt['datagroup'] = 3 - ind
        best_model_to_csv(param_dict_gbt, 'zzzgbtll_rv')
        res = pd.DataFrame(gbt.cv_results_)
        df = pd.concat([df, res])
        df.to_csv("zzzgbtll_cv_rv.csv")
        joblib.dump(gbt, 'grid_gbt_rv{}.pkl'.format(3-ind))
